(function(){
  ...
  var toggler = document.getElementById('catalog-nav-toggler');
  if(toggler){
    ...
    function mainNavVisibleToggle(e) {
      ...
      document.getElementById('catalog-nav').classList.toggle('catalog-nav--open');
    }
  }

  // Добавление/удаление модификаторов при фокусировке на ссылочном элементе
  var linkClassName = 'catalog-nav__link';
  var linkClassNameShowChild = 'catalog-nav__item--show-child';
  ...
  document.addEventListener('focus', function(event) {
    ...
    if (findLinkClassName.test(event.target.className)) {
      ...
      event.target.parents('.catalog-nav__item').forEach(function(item){
  ...
  document.addEventListener('blur', function(event) {
    ...
    if (findLinkClassName.test(event.target.className)) {
      ...
      // event.target.closest('.catalog-nav').querySelectorAll('.'+linkClassNameShowChild).forEach(function(item){
